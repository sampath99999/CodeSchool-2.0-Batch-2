$(document).ready(() => {
  ...
  $.get(
    ...
    (data, status) => {
      ...
      // Extracting common parts of the code
        ...
        const createPostCard = (post) => {
            ...
            return `
              <div class="postCard border border-light-subtle mb-4 rounded shadow-sm">
                ...
                <p class="pe-2 ps-2 m-0">${post.author} <span>.</span> ${formatDate(date)} <span>.</span> <i class="fa-regular fa-comment"></i> (0)</p>
                ...
                <div class="postCardFooter d-flex justify-content-between pe-2 ps-2 align-items-center pb-2">
                  ...
                  <div>
                    <button type="button" class="border-0 bg-transparent pt-md-3" data-bs-toggle="modal" data-bs-target="#staticBackdrop${post.publishedAt}">
                      <p class="d-none d-md-flex modalBtn align-items-center">Continue reading <span class="material-symbols-outlined">keyboard_double_arrow_right</span></p>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <!-- Modal -->
              <div class="modal fade" id="staticBackdrop${post.publishedAt}" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-xl">
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <img src="${post.urlToImage}" class="w-100 rounded mb-3" />
      ...
      loadPosts(articles, 1); // Load the first page of posts initially

      // Add pagination click event handlers
      ...
      const loadFooterImages = (postsList) => {
        ...
        postsList.slice(0, 6).forEach((post) => {
          const { urlToImage } = post;
          $(".footerImgContainer").append(
            `<img src="${urlToImage}" alt="footerImg" class="rounded shadow-sm" style="width:168px" />`
          );
        });
      };

      loadPosts(articles.slice(31, 41));
      loadPopularPosts(articles.slice(31, 34));
      loadCelebrationItem(articles[51]);
      loadTagClouds(articles.slice(51, 61));
      loadFooterImages(articles.slice(61, 71));
