...
function validatePartyAccountNumber(accountNumber) {
  ...
  if (accountNumber.substring(1).includes("0")) {
    error.push(
      "Party Account number should not have additional zeros after the first digit."
    );
  }
...
function createUploadDocs() {
  ...
  uploadDocsButton.click(function () {
    uploadDocsInput.click();
  });
  uploadDocsInput.change(function () {
...
function convertToWords(number) {
  ...
  const numToWords = (num) => {
    ...
    for (let i = 0; num > 0; i++) {
      if (num % 1000 !== 0) {
        words =
          numToWordsUnderThousand(num % 1000) + thousands[i] + " " + words;
      }
  ...
  const numToWordsUnderThousand = (num) => {
    if (num === 0) return "";
    ...
    return (
      ...
      " Hundred " +
      numToWordsUnderThousand(num % 100)
    );
  };

  return numToWords(Number(number));
}
